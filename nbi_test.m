...
% rx_fft = fft(y, N_sym, 2);
% P_signal = sum(abs(rx_fft) .^ 2, 2);
% 
% % % 对每个符号的子载波根据SINR排序
...
for j = 1:N_sym
    ...
    for i = 1:M
        ...
        s_hat = qamdemod(z(i, j), Mod_Order, 'UnitAveragePower', true); % 解调为原始调制符号
        % 计算干扰残差
